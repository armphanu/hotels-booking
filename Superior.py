...
class superior(object):
    ...
    def status(self):
        ...
        self.cur.execute("select status from superior where rowid=(?)", (a, ))
        if a != "-":
    ...
    def connecter(self):
        ...
        if c == 1:
            ...
            self.window.show()
            self.cur.execute("""Update superior set status = 0 where rowid=(?)""", (a, ))
            self.conn.commit()
            self.pay_2.setText("FULL")
        else:
            self.window = QtWidgets.QMainWindow()
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_8.setText(_translate("MainWindow", "Superior Room"))
